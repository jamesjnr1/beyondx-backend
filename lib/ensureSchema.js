...
const WORKER_COLUMNS = [
  ...
  // Backs the "brings own tools" +15% dispatch surcharge and the "Has
  // tools" badge — the frontend has read/written this field for a while
  // (ProfileModal, EmployerDashboard) but no column ever existed for it.
  `ALTER TABLE "Worker" ADD COLUMN IF NOT EXISTS "hasTools" BOOLEAN NOT NULL DEFAULT false`,
  // Work Experience & Certifications card (WorkerDashboard.tsx) — same
  // story: read/written via PATCH /api/workers/me for a while with no
  // column behind either one.
  ...
  // Manual staff placement boost + optional client testimonial — see
  // schema.prisma for the full explanation of both.
  `ALTER TABLE "Worker" ADD COLUMN IF NOT EXISTS "priority" BOOLEAN NOT NULL DEFAULT false`,
  `ALTER TABLE "Worker" ADD COLUMN IF NOT EXISTS "featuredTestimonial" TEXT`,
  `ALTER TABLE "Worker" ADD COLUMN IF NOT EXISTS "featuredTestimonialSource" TEXT`,
];
...
// PushSubscription: one row per device that granted notification permission.
// Brand new table, same story as CoordinatorJobRequest above — created here
// rather than through a migration that will never actually run against the
// live database. Unlike that table, a subscription is disposable data tied
// to a device, not a business record, so its owner FKs cascade on delete
// instead of restricting it (deleting a worker/employer should never be
// blocked by their old push subscriptions).
const PUSH_SUBSCRIPTION_STATEMENTS = [
  ...
  `CREATE INDEX IF NOT EXISTS "PushSubscription_workerId_idx" ON "PushSubscription"("workerId")`,
  `CREATE INDEX IF NOT EXISTS "PushSubscription_employerId_idx" ON "PushSubscription"("employerId")`,
  `ALTER TABLE "PushSubscription" ADD CONSTRAINT "PushSubscription_workerId_fkey"
    FOREIGN KEY ("workerId") REFERENCES "Worker"("id") ON DELETE CASCADE ON UPDATE CASCADE`,
  `ALTER TABLE "PushSubscription" ADD CONSTRAINT "PushSubscription_employerId_fkey"
    FOREIGN KEY ("employerId") REFERENCES "Employer"("id") ON DELETE CASCADE ON UPDATE CASCADE`,
];

async function ensurePushSubscriptionTable(prisma) {
  for (const sql of PUSH_SUBSCRIPTION_STATEMENTS) {
    try {
      await prisma.$executeRawUnsafe(sql);
    } catch (err) {
      console.error('[ensureSchema] failed to apply:', sql.split('\n')[0], err.message);
...
async function ensureSchema(prisma) {
  ...
  await ensurePushSubscriptionTable(prisma);
}

module.exports = {
  ensureWorkerColumns, ensureTaskColumns, ensureCoordinatorJobRequestsTable,
  ensurePushSubscriptionTable, ensureSchema,
};
