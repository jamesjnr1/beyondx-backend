...
router.get('/health', async (req, res) => {
  try {
    ...
    res.json({ ok: true, message: 'OTP route live.' });
  } catch (err) {
...
router.post('/send', async (req, res) => {
  ...
  try {
    ...
    if (existing) {
      ...
      if (age < COOLDOWN) {
        const wait = Math.ceil((COOLDOWN - age) / 1000);
    ...
    await prisma.phoneOtp.upsert({
      where:  { phone },
      update: { code, expiresAt: new Date(Date.now() + TTL_MS), createdAt: new Date() },
      create: { phone, code, expiresAt: new Date(Date.now() + TTL_MS) },
    });

    // Use sendSMS — same function that sends all other SMS in the app,
    // uses the approved BeyondX sender ID, logs to admin SMS console.
    const displayPhone = '0' + phone.slice(3);
    await sendSMS(displayPhone, `BeyondX: ${code} is your verification code. Valid for 15 minutes.`);
...
router.post('/verify', async (req, res) => {
  ...
  try {
    ...
    if (row.code !== code)                 return res.status(400).json({ error: 'Incorrect code. Please try again.' });
